"""
Vortex Streaming Feature Pipeline: item_popularity_decay_pipeline
Updates time-decayed item CTR and conversion rates on streaming clicks
"""
...
logger = get_logger("feature_store.pipeline.item_popularity_decay_pipeline")

class ItemPopularityDecayPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class ItemPopularityDecayPipeline:
    """Updates time-decayed item CTR and conversion rates on streaming clicks"""

    def __init__(self, config: Optional[ItemPopularityDecayPipelineConfig] = None):
        self.config = config or ItemPopularityDecayPipelineConfig()
