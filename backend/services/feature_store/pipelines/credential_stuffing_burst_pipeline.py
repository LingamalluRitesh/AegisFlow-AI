"""
Vortex Streaming Feature Pipeline: credential_stuffing_burst_pipeline
Aggregates global authentication failure spikes across edge IPs
"""
...
logger = get_logger("feature_store.pipeline.credential_stuffing_burst_pipeline")

class CredentialStuffingBurstPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class CredentialStuffingBurstPipeline:
    """Aggregates global authentication failure spikes across edge IPs"""

    def __init__(self, config: Optional[CredentialStuffingBurstPipelineConfig] = None):
        self.config = config or CredentialStuffingBurstPipelineConfig()
