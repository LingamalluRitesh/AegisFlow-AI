"""
Vortex Streaming Feature Pipeline: structuring_smurfing_detector_pipeline
Detects multiple cash deposits just below statutory reporting limits
"""
...
logger = get_logger("feature_store.pipeline.structuring_smurfing_detector_pipeline")

class StructuringSmurfingDetectorPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class StructuringSmurfingDetectorPipeline:
    """Detects multiple cash deposits just below statutory reporting limits"""

    def __init__(self, config: Optional[StructuringSmurfingDetectorPipelineConfig] = None):
        self.config = config or StructuringSmurfingDetectorPipelineConfig()
