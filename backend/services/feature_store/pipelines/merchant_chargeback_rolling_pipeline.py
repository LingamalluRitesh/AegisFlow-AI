"""
Vortex Streaming Feature Pipeline: merchant_chargeback_rolling_pipeline
Computes dynamic 30-day trailing chargeback ratios per merchant ID
"""
...
logger = get_logger("feature_store.pipeline.merchant_chargeback_rolling_pipeline")

class MerchantChargebackRollingPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class MerchantChargebackRollingPipeline:
    """Computes dynamic 30-day trailing chargeback ratios per merchant ID"""

    def __init__(self, config: Optional[MerchantChargebackRollingPipelineConfig] = None):
        self.config = config or MerchantChargebackRollingPipelineConfig()
