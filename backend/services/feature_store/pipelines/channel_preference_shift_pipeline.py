"""
Vortex Streaming Feature Pipeline: channel_preference_shift_pipeline
Detects abrupt transitions between native mobile app and web browser channels
"""
...
logger = get_logger("feature_store.pipeline.channel_preference_shift_pipeline")

class ChannelPreferenceShiftPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class ChannelPreferenceShiftPipeline:
    """Detects abrupt transitions between native mobile app and web browser channels"""

    def __init__(self, config: Optional[ChannelPreferenceShiftPipelineConfig] = None):
        self.config = config or ChannelPreferenceShiftPipelineConfig()
