"""
Vortex Streaming Feature Pipeline: chargeback_bayesian_prior_pipeline
Updates empirical Bayes risk distributions upon receiving chargeback notices
"""
...
logger = get_logger("feature_store.pipeline.chargeback_bayesian_prior_pipeline")

class ChargebackBayesianPriorPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class ChargebackBayesianPriorPipeline:
    """Updates empirical Bayes risk distributions upon receiving chargeback notices"""

    def __init__(self, config: Optional[ChargebackBayesianPriorPipelineConfig] = None):
        self.config = config or ChargebackBayesianPriorPipelineConfig()
