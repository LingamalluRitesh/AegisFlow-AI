"""
Vortex Streaming Feature Pipeline: circadian_rhythm_deviation_pipeline
Calculates z-score of transaction time relative to historical diurnal patterns
"""
...
logger = get_logger("feature_store.pipeline.circadian_rhythm_deviation_pipeline")

class CircadianRhythmDeviationPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class CircadianRhythmDeviationPipeline:
    """Calculates z-score of transaction time relative to historical diurnal patterns"""

    def __init__(self, config: Optional[CircadianRhythmDeviationPipelineConfig] = None):
        self.config = config or CircadianRhythmDeviationPipelineConfig()
