"""
Vortex Streaming Feature Pipeline: card_authorization_frequency_pipeline
Maintains high-frequency PAN micro-authorization counters
"""
...
logger = get_logger("feature_store.pipeline.card_authorization_frequency_pipeline")

class CardAuthorizationFrequencyPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class CardAuthorizationFrequencyPipeline:
    """Maintains high-frequency PAN micro-authorization counters"""

    def __init__(self, config: Optional[CardAuthorizationFrequencyPipelineConfig] = None):
        self.config = config or CardAuthorizationFrequencyPipelineConfig()
