"""
Vortex Streaming Feature Pipeline: loyalty_points_drain_pipeline
Monitors high-speed redemption of reward balances following account access
"""
...
logger = get_logger("feature_store.pipeline.loyalty_points_drain_pipeline")

class LoyaltyPointsDrainPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class LoyaltyPointsDrainPipeline:
    """Monitors high-speed redemption of reward balances following account access"""

    def __init__(self, config: Optional[LoyaltyPointsDrainPipelineConfig] = None):
        self.config = config or LoyaltyPointsDrainPipelineConfig()
