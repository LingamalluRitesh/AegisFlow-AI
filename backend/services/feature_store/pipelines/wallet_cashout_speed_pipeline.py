"""
Vortex Streaming Feature Pipeline: wallet_cashout_speed_pipeline
Measures duration between deposit clearing and external fiat withdrawal
"""
...
logger = get_logger("feature_store.pipeline.wallet_cashout_speed_pipeline")

class WalletCashoutSpeedPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class WalletCashoutSpeedPipeline:
    """Measures duration between deposit clearing and external fiat withdrawal"""

    def __init__(self, config: Optional[WalletCashoutSpeedPipelineConfig] = None):
        self.config = config or WalletCashoutSpeedPipelineConfig()
