"""
Vortex Streaming Feature Pipeline: merchandise_return_velocity_pipeline
Maintains trailing refund dollar volumes relative to net purchases
"""
...
logger = get_logger("feature_store.pipeline.merchandise_return_velocity_pipeline")

class MerchandiseReturnVelocityPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class MerchandiseReturnVelocityPipeline:
    """Maintains trailing refund dollar volumes relative to net purchases"""

    def __init__(self, config: Optional[MerchandiseReturnVelocityPipelineConfig] = None):
        self.config = config or MerchandiseReturnVelocityPipelineConfig()
