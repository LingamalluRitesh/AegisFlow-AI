"""
Vortex Streaming Feature Pipeline: user_transaction_velocity_pipeline
Aggregates continuous sliding window velocity across multiple time grains
"""
...
logger = get_logger("feature_store.pipeline.user_transaction_velocity_pipeline")

class UserTransactionVelocityPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class UserTransactionVelocityPipeline:
    """Aggregates continuous sliding window velocity across multiple time grains"""

    def __init__(self, config: Optional[UserTransactionVelocityPipelineConfig] = None):
        self.config = config or UserTransactionVelocityPipelineConfig()
