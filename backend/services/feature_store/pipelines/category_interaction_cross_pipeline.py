"""
Vortex Streaming Feature Pipeline: category_interaction_cross_pipeline
Constructs real-time sparse user-category interaction matrices
"""
...
logger = get_logger("feature_store.pipeline.category_interaction_cross_pipeline")

class CategoryInteractionCrossPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class CategoryInteractionCrossPipeline:
    """Constructs real-time sparse user-category interaction matrices"""

    def __init__(self, config: Optional[CategoryInteractionCrossPipelineConfig] = None):
        self.config = config or CategoryInteractionCrossPipelineConfig()
