"""
Vortex Streaming Feature Pipeline: address_leap_verification_pipeline
Computes geographic distance between shipping and billing address coordinates
"""
...
logger = get_logger("feature_store.pipeline.address_leap_verification_pipeline")

class AddressLeapVerificationPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class AddressLeapVerificationPipeline:
    """Computes geographic distance between shipping and billing address coordinates"""

    def __init__(self, config: Optional[AddressLeapVerificationPipelineConfig] = None):
        self.config = config or AddressLeapVerificationPipelineConfig()
