"""
Vortex Streaming Feature Pipeline: ip_geodistance_leap_pipeline
Computes real-time haversine distance deltas between sequential logins
"""
...
logger = get_logger("feature_store.pipeline.ip_geodistance_leap_pipeline")

class IpGeodistanceLeapPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class IpGeodistanceLeapPipeline:
    """Computes real-time haversine distance deltas between sequential logins"""

    def __init__(self, config: Optional[IpGeodistanceLeapPipelineConfig] = None):
        self.config = config or IpGeodistanceLeapPipelineConfig()
