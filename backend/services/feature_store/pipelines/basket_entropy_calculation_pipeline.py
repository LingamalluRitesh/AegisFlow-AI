"""
Vortex Streaming Feature Pipeline: basket_entropy_calculation_pipeline
Computes information entropy over SKU product category distributions
"""
...
logger = get_logger("feature_store.pipeline.basket_entropy_calculation_pipeline")

class BasketEntropyCalculationPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class BasketEntropyCalculationPipeline:
    """Computes information entropy over SKU product category distributions"""

    def __init__(self, config: Optional[BasketEntropyCalculationPipelineConfig] = None):
        self.config = config or BasketEntropyCalculationPipelineConfig()
