"""
Vortex Streaming Feature Pipeline: biometric_timing_entropy_pipeline
Computes variance of inter-keystroke timing intervals
"""
...
logger = get_logger("feature_store.pipeline.biometric_timing_entropy_pipeline")

class BiometricTimingEntropyPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class BiometricTimingEntropyPipeline:
    """Computes variance of inter-keystroke timing intervals"""

    def __init__(self, config: Optional[BiometricTimingEntropyPipelineConfig] = None):
        self.config = config or BiometricTimingEntropyPipelineConfig()
