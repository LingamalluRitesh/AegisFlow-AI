"""
Vortex Streaming Feature Pipeline: sanctions_fuzzy_distance_pipeline
Computes Levenshtein and Jaro-Winkler distances against sanctions lists
"""
...
logger = get_logger("feature_store.pipeline.sanctions_fuzzy_distance_pipeline")

class SanctionsFuzzyDistancePipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class SanctionsFuzzyDistancePipeline:
    """Computes Levenshtein and Jaro-Winkler distances against sanctions lists"""

    def __init__(self, config: Optional[SanctionsFuzzyDistancePipelineConfig] = None):
        self.config = config or SanctionsFuzzyDistancePipelineConfig()
