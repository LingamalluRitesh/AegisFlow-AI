"""
Vortex Streaming Feature Pipeline: multi_card_linkage_pipeline
Tracks count and issuer diversity of payment methods added within 1 hour
"""
...
logger = get_logger("feature_store.pipeline.multi_card_linkage_pipeline")

class MultiCardLinkagePipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class MultiCardLinkagePipeline:
    """Tracks count and issuer diversity of payment methods added within 1 hour"""

    def __init__(self, config: Optional[MultiCardLinkagePipelineConfig] = None):
        self.config = config or MultiCardLinkagePipelineConfig()
