"""
Vortex Streaming Feature Pipeline: crypto_mixing_proximity_pipeline
Evaluates graph distance to known darknet and sanctioned wallet addresses
"""
...
logger = get_logger("feature_store.pipeline.crypto_mixing_proximity_pipeline")

class CryptoMixingProximityPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class CryptoMixingProximityPipeline:
    """Evaluates graph distance to known darknet and sanctioned wallet addresses"""

    def __init__(self, config: Optional[CryptoMixingProximityPipelineConfig] = None):
        self.config = config or CryptoMixingProximityPipelineConfig()
