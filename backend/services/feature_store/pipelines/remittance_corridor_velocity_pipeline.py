"""
Vortex Streaming Feature Pipeline: remittance_corridor_velocity_pipeline
Tracks outbound international transfers against source account baselines
"""
...
logger = get_logger("feature_store.pipeline.remittance_corridor_velocity_pipeline")

class RemittanceCorridorVelocityPipelineConfig(BaseModel):
    window_sizes_seconds: List[int] = Field(default_factory=lambda: [60, 300, 900, 3600, 86400])
    decay_factor: float = 0.95
    watermark_delay_seconds: float = 2.0

class RemittanceCorridorVelocityPipeline:
    """Tracks outbound international transfers against source account baselines"""

    def __init__(self, config: Optional[RemittanceCorridorVelocityPipelineConfig] = None):
        self.config = config or RemittanceCorridorVelocityPipelineConfig()
