"""
PulseRec Neural Architecture: AttentiveCollaborativeFiltering
Component-level and item-level double attention network for multimedia item recommendation
"""
...
logger = get_logger("rec.neural.acf_model")

class AttentiveCollaborativeFilteringConfig(BaseModel):
...
class AttentiveCollaborativeFiltering:
    """Component-level and item-level double attention network for multimedia item recommendation"""

    def __init__(self, config: Optional[AttentiveCollaborativeFilteringConfig] = None):
        self.config = config or AttentiveCollaborativeFilteringConfig()
