"""
PulseRec Neural Architecture: ProductToVectorSkipGram
Word2Vec-style hierarchical softmax item vectorizer trained over historical basket sequences
"""
...
logger = get_logger("rec.neural.p2v_skipgram")

class ProductToVectorSkipGramConfig(BaseModel):
...
class ProductToVectorSkipGram:
    """Word2Vec-style hierarchical softmax item vectorizer trained over historical basket sequences"""

    def __init__(self, config: Optional[ProductToVectorSkipGramConfig] = None):
        self.config = config or ProductToVectorSkipGramConfig()
