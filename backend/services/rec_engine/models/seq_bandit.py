"""
PulseRec Neural Architecture: SequentialContextualBandit
Sequential Contextual Multi-Armed Bandit with stateful RNN-augmented LinUCB exploration
"""
...
logger = get_logger("rec.neural.seq_bandit")

class SequentialContextualBanditConfig(BaseModel):
...
class SequentialContextualBandit:
    """Sequential Contextual Multi-Armed Bandit with stateful RNN-augmented LinUCB exploration"""

    def __init__(self, config: Optional[SequentialContextualBanditConfig] = None):
        self.config = config or SequentialContextualBanditConfig()
