"""
PulseRec Neural Architecture: CategoryTaxonomyEmbeddingNet
Deep metric learning network preserving hierarchical tree distance in latent embedding spaces
"""
...
logger = get_logger("rec.neural.category_emb_net")

class CategoryTaxonomyEmbeddingNetConfig(BaseModel):
...
class CategoryTaxonomyEmbeddingNet:
    """Deep metric learning network preserving hierarchical tree distance in latent embedding spaces"""

    def __init__(self, config: Optional[CategoryTaxonomyEmbeddingNetConfig] = None):
        self.config = config or CategoryTaxonomyEmbeddingNetConfig()
