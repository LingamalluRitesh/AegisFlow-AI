"""
PulseRec Neural Architecture: GraphConvolutionalRecNet
Graph Convolutional Network (GCN) propagating high-order collaborative signals on bipartite user-item graphs
"""
...
logger = get_logger("rec.neural.gcn_rec")

class GraphConvolutionalRecNetConfig(BaseModel):
...
class GraphConvolutionalRecNet:
    """Graph Convolutional Network (GCN) propagating high-order collaborative signals on bipartite user-item graphs"""

    def __init__(self, config: Optional[GraphConvolutionalRecNetConfig] = None):
        self.config = config or GraphConvolutionalRecNetConfig()
