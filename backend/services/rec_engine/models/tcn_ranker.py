"""
PulseRec Neural Architecture: TransformerCrossNetwork
Transformer Cross Network combining token self-attention with explicit feature-crossing layers
"""
...
logger = get_logger("rec.neural.tcn_ranker")

class TransformerCrossNetworkConfig(BaseModel):
...
class TransformerCrossNetwork:
    """Transformer Cross Network combining token self-attention with explicit feature-crossing layers"""

    def __init__(self, config: Optional[TransformerCrossNetworkConfig] = None):
        self.config = config or TransformerCrossNetworkConfig()
