"""
AegisGuard Enterprise Detector: SanctionsScreeningDetector
Fuzzy-matches OFAC, PEP, and international sanctions watchlists
"""
...
logger = get_logger("fraud.detector.sanctions_check")

class SanctionsScreeningDetectorConfig(BaseModel):
    enabled: bool = True
...
class SanctionsScreeningDetectorResult(BaseModel):
    detector_name: str = "SanctionsScreeningDetector"
    risk_score: float
...
class SanctionsScreeningDetector:
    """Fuzzy-matches OFAC, PEP, and international sanctions watchlists"""

    def __init__(self, config: Optional[SanctionsScreeningDetectorConfig] = None):
        self.config = config or SanctionsScreeningDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> SanctionsScreeningDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return SanctionsScreeningDetectorResult(
                risk_score=0.0,
