"""
AegisGuard Enterprise Detector: ACHReturnDetector
Predicts NSF (Non-Sufficient Funds) and unauthorized ACH debit returns
"""
...
logger = get_logger("fraud.detector.ach_return")

class ACHReturnDetectorConfig(BaseModel):
    enabled: bool = True
...
class ACHReturnDetectorResult(BaseModel):
    detector_name: str = "ACHReturnDetector"
    risk_score: float
...
class ACHReturnDetector:
    """Predicts NSF (Non-Sufficient Funds) and unauthorized ACH debit returns"""

    def __init__(self, config: Optional[ACHReturnDetectorConfig] = None):
        self.config = config or ACHReturnDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> ACHReturnDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return ACHReturnDetectorResult(
                risk_score=0.0,
