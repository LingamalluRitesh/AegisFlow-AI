"""
AegisGuard Enterprise Detector: AffiliateFraudDetector
Uncovers referral manipulation and illegitimate conversion attributions
"""
...
logger = get_logger("fraud.detector.affiliate_fraud")

class AffiliateFraudDetectorConfig(BaseModel):
    enabled: bool = True
...
class AffiliateFraudDetectorResult(BaseModel):
    detector_name: str = "AffiliateFraudDetector"
    risk_score: float
...
class AffiliateFraudDetector:
    ...
    def __init__(self, config: Optional[AffiliateFraudDetectorConfig] = None):
        self.config = config or AffiliateFraudDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> AffiliateFraudDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return AffiliateFraudDetectorResult(
                risk_score=0.0,
