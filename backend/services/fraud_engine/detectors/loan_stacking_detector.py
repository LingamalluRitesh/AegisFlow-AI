"""
AegisGuard Enterprise Detector: LoanStackingDetector
Detects simultaneous multi-lender credit applications within minutes
"""
...
logger = get_logger("fraud.detector.loan_stacking")

class LoanStackingDetectorConfig(BaseModel):
    enabled: bool = True
...
class LoanStackingDetectorResult(BaseModel):
    detector_name: str = "LoanStackingDetector"
    risk_score: float
...
class LoanStackingDetector:
    ...
    def __init__(self, config: Optional[LoanStackingDetectorConfig] = None):
        self.config = config or LoanStackingDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> LoanStackingDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return LoanStackingDetectorResult(
                risk_score=0.0,
