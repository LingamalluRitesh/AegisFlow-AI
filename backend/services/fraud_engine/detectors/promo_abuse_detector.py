"""
AegisGuard Enterprise Detector: PromoAbuseDetector
Prevents multi-accounting for promo code and voucher exploitation
"""
...
logger = get_logger("fraud.detector.promo_abuse")

class PromoAbuseDetectorConfig(BaseModel):
    enabled: bool = True
...
class PromoAbuseDetectorResult(BaseModel):
    detector_name: str = "PromoAbuseDetector"
    risk_score: float
...
class PromoAbuseDetector:
    ...
    def __init__(self, config: Optional[PromoAbuseDetectorConfig] = None):
        self.config = config or PromoAbuseDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> PromoAbuseDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return PromoAbuseDetectorResult(
                risk_score=0.0,
