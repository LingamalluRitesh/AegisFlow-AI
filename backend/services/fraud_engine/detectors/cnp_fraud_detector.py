"""
AegisGuard Enterprise Detector: CardNotPresentDetector
Evaluates e-commerce CNP payment vectors, CVC mismatch, and billing anomalies
"""
...
logger = get_logger("fraud.detector.cnp_fraud")

class CardNotPresentDetectorConfig(BaseModel):
    enabled: bool = True
...
class CardNotPresentDetectorResult(BaseModel):
    detector_name: str = "CardNotPresentDetector"
    risk_score: float
...
class CardNotPresentDetector:
    """Evaluates e-commerce CNP payment vectors, CVC mismatch, and billing anomalies"""

    def __init__(self, config: Optional[CardNotPresentDetectorConfig] = None):
        self.config = config or CardNotPresentDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> CardNotPresentDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return CardNotPresentDetectorResult(
                risk_score=0.0,
