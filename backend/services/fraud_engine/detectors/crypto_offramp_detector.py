"""
AegisGuard Enterprise Detector: CryptoOfframpDetector
Monitors high-risk crypto mixer interactions and rapid fiat off-ramping
"""
...
logger = get_logger("fraud.detector.crypto_offramp")

class CryptoOfframpDetectorConfig(BaseModel):
    enabled: bool = True
...
class CryptoOfframpDetectorResult(BaseModel):
    detector_name: str = "CryptoOfframpDetector"
    risk_score: float
...
class CryptoOfframpDetector:
    """Monitors high-risk crypto mixer interactions and rapid fiat off-ramping"""

    def __init__(self, config: Optional[CryptoOfframpDetectorConfig] = None):
        self.config = config or CryptoOfframpDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> CryptoOfframpDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return CryptoOfframpDetectorResult(
                risk_score=0.0,
