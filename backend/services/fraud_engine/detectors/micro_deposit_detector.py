"""
AegisGuard Enterprise Detector: MicroDepositDetector
Mitigates automated bank verification probing and micro-deposit enumeration
"""
...
logger = get_logger("fraud.detector.micro_deposit")

class MicroDepositDetectorConfig(BaseModel):
    enabled: bool = True
...
class MicroDepositDetectorResult(BaseModel):
    detector_name: str = "MicroDepositDetector"
    risk_score: float
...
class MicroDepositDetector:
    """Mitigates automated bank verification probing and micro-deposit enumeration"""

    def __init__(self, config: Optional[MicroDepositDetectorConfig] = None):
        self.config = config or MicroDepositDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> MicroDepositDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return MicroDepositDetectorResult(
                risk_score=0.0,
