"""
AegisGuard Enterprise Detector: FriendlyFraudDetector
Identifies false claims of unauthorized charges by legitimate cardholders
"""
...
logger = get_logger("fraud.detector.friendly_fraud")

class FriendlyFraudDetectorConfig(BaseModel):
    enabled: bool = True
...
class FriendlyFraudDetectorResult(BaseModel):
    detector_name: str = "FriendlyFraudDetector"
    risk_score: float
...
class FriendlyFraudDetector:
    ...
    def __init__(self, config: Optional[FriendlyFraudDetectorConfig] = None):
        self.config = config or FriendlyFraudDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> FriendlyFraudDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return FriendlyFraudDetectorResult(
                risk_score=0.0,
