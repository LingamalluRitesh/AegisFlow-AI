"""
AegisGuard Enterprise Detector: MuleAccountDetector
Detects mule accounts used for money laundering and rapid fund dispersion
"""
...
logger = get_logger("fraud.detector.mule_account")

class MuleAccountDetectorConfig(BaseModel):
    enabled: bool = True
...
class MuleAccountDetectorResult(BaseModel):
    detector_name: str = "MuleAccountDetector"
    risk_score: float
...
class MuleAccountDetector:
    ...
    def __init__(self, config: Optional[MuleAccountDetectorConfig] = None):
        self.config = config or MuleAccountDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> MuleAccountDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return MuleAccountDetectorResult(
                risk_score=0.0,
