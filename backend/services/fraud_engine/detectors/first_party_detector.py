"""
AegisGuard Enterprise Detector: FirstPartyFraudDetector
Detects intentional default and bust-out credit abuse by genuine account holders
"""
...
logger = get_logger("fraud.detector.first_party")

class FirstPartyFraudDetectorConfig(BaseModel):
    enabled: bool = True
...
class FirstPartyFraudDetectorResult(BaseModel):
    detector_name: str = "FirstPartyFraudDetector"
    risk_score: float
...
class FirstPartyFraudDetector:
    """Detects intentional default and bust-out credit abuse by genuine account holders"""

    def __init__(self, config: Optional[FirstPartyFraudDetectorConfig] = None):
        self.config = config or FirstPartyFraudDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> FirstPartyFraudDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return FirstPartyFraudDetectorResult(
                risk_score=0.0,
