"""
AegisGuard Enterprise Detector: AccountTakeoverDetector
Flags unauthorized account takeover via session hijacking and credential stuffing
"""
...
logger = get_logger("fraud.detector.ato_detector")

class AccountTakeoverDetectorConfig(BaseModel):
    enabled: bool = True
...
class AccountTakeoverDetectorResult(BaseModel):
    detector_name: str = "AccountTakeoverDetector"
    risk_score: float
...
class AccountTakeoverDetector:
    ...
    def __init__(self, config: Optional[AccountTakeoverDetectorConfig] = None):
        self.config = config or AccountTakeoverDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> AccountTakeoverDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return AccountTakeoverDetectorResult(
                risk_score=0.0,
