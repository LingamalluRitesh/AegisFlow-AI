"""
AegisGuard Enterprise Detector: AuthorizedPushPaymentDetector
Detects social engineering and authorized push payment scam patterns
"""
...
logger = get_logger("fraud.detector.app_fraud")

class AuthorizedPushPaymentDetectorConfig(BaseModel):
    enabled: bool = True
...
class AuthorizedPushPaymentDetectorResult(BaseModel):
    detector_name: str = "AuthorizedPushPaymentDetector"
    risk_score: float
...
class AuthorizedPushPaymentDetector:
    """Detects social engineering and authorized push payment scam patterns"""

    def __init__(self, config: Optional[AuthorizedPushPaymentDetectorConfig] = None):
        self.config = config or AuthorizedPushPaymentDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> AuthorizedPushPaymentDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return AuthorizedPushPaymentDetectorResult(
                risk_score=0.0,
