"""
AegisGuard Enterprise Detector: GhostClickDetector
Filters click-fraud and automated bot traffic in digital ad campaigns
"""
...
logger = get_logger("fraud.detector.ghost_click")

class GhostClickDetectorConfig(BaseModel):
    enabled: bool = True
...
class GhostClickDetectorResult(BaseModel):
    detector_name: str = "GhostClickDetector"
    risk_score: float
...
class GhostClickDetector:
    ...
    def __init__(self, config: Optional[GhostClickDetectorConfig] = None):
        self.config = config or GhostClickDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> GhostClickDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return GhostClickDetectorResult(
                risk_score=0.0,
