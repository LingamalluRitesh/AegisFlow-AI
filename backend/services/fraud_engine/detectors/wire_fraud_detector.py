"""
AegisGuard Enterprise Detector: WireFraudDetector
Intersects high-value wire transfers and BEC (Business Email Compromise) patterns
"""
...
logger = get_logger("fraud.detector.wire_fraud")

class WireFraudDetectorConfig(BaseModel):
    enabled: bool = True
...
class WireFraudDetectorResult(BaseModel):
    detector_name: str = "WireFraudDetector"
    risk_score: float
...
class WireFraudDetector:
    ...
    def __init__(self, config: Optional[WireFraudDetectorConfig] = None):
        self.config = config or WireFraudDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> WireFraudDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return WireFraudDetectorResult(
                risk_score=0.0,
