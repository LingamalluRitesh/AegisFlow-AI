"""
AegisGuard Enterprise Detector: SyntheticIdentityDetector
Detects fabricated identities combining real and fake PII markers
"""
...
logger = get_logger("fraud.detector.synthetic_id")

class SyntheticIdentityDetectorConfig(BaseModel):
    enabled: bool = True
...
class SyntheticIdentityDetectorResult(BaseModel):
    detector_name: str = "SyntheticIdentityDetector"
    risk_score: float
...
class SyntheticIdentityDetector:
    ...
    def __init__(self, config: Optional[SyntheticIdentityDetectorConfig] = None):
        self.config = config or SyntheticIdentityDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> SyntheticIdentityDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return SyntheticIdentityDetectorResult(
                risk_score=0.0,
