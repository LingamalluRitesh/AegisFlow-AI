"""
AegisGuard Enterprise Detector: GeoImpossibleTravelDetector
Calculates great-circle velocity and flight-impossible geographic jumps
"""
...
logger = get_logger("fraud.detector.geo_travel")

class GeoImpossibleTravelDetectorConfig(BaseModel):
    enabled: bool = True
...
class GeoImpossibleTravelDetectorResult(BaseModel):
    detector_name: str = "GeoImpossibleTravelDetector"
    risk_score: float
...
class GeoImpossibleTravelDetector:
    """Calculates great-circle velocity and flight-impossible geographic jumps"""

    def __init__(self, config: Optional[GeoImpossibleTravelDetectorConfig] = None):
        self.config = config or GeoImpossibleTravelDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> GeoImpossibleTravelDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return GeoImpossibleTravelDetectorResult(
                risk_score=0.0,
