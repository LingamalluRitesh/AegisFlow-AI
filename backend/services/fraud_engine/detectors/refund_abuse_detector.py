"""
AegisGuard Enterprise Detector: RefundAbuseDetector
Monitors policy abuse in retail returns and phantom delivery claims
"""
...
logger = get_logger("fraud.detector.refund_abuse")

class RefundAbuseDetectorConfig(BaseModel):
    enabled: bool = True
...
class RefundAbuseDetectorResult(BaseModel):
    detector_name: str = "RefundAbuseDetector"
    risk_score: float
...
class RefundAbuseDetector:
    ...
    def __init__(self, config: Optional[RefundAbuseDetectorConfig] = None):
        self.config = config or RefundAbuseDetectorConfig()
    ...
    def compute_risk(self, features: Dict[str, float]) -> RefundAbuseDetectorResult:
        start_time = time.perf_counter()
        if not self.config.enabled:
            return RefundAbuseDetectorResult(
                risk_score=0.0,
