"""
Integration Scenario Test Matrix #05
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_05():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0005",
        user_id="usr_scenario_0005",
        source_account_id="acct_src_0005",
        target_account_id="acct_tgt_0005",
        amount=55.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0005"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0005", candidate_count=4)
