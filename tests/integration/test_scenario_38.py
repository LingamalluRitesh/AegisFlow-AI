"""
Integration Scenario Test Matrix #38
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_38():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0038",
        user_id="usr_scenario_0038",
        source_account_id="acct_src_0038",
        target_account_id="acct_tgt_0038",
        amount=385.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0038"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0038", candidate_count=4)
