"""
Integration Scenario Test Matrix #07
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_07():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0007",
        user_id="usr_scenario_0007",
        source_account_id="acct_src_0007",
        target_account_id="acct_tgt_0007",
        amount=75.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0007"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0007", candidate_count=4)
