"""
Integration Scenario Test Matrix #13
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_13():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0013",
        user_id="usr_scenario_0013",
        source_account_id="acct_src_0013",
        target_account_id="acct_tgt_0013",
        amount=135.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0013"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0013", candidate_count=4)
