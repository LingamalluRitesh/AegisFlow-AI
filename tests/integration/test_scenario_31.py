"""
Integration Scenario Test Matrix #31
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_31():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0031",
        user_id="usr_scenario_0031",
        source_account_id="acct_src_0031",
        target_account_id="acct_tgt_0031",
        amount=315.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0031"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0031", candidate_count=4)
