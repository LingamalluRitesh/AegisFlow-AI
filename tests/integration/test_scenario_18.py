"""
Integration Scenario Test Matrix #18
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_18():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0018",
        user_id="usr_scenario_0018",
        source_account_id="acct_src_0018",
        target_account_id="acct_tgt_0018",
        amount=185.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0018"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0018", candidate_count=4)
