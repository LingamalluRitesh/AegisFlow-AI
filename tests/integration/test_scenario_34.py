"""
Integration Scenario Test Matrix #34
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_34():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0034",
        user_id="usr_scenario_0034",
        source_account_id="acct_src_0034",
        target_account_id="acct_tgt_0034",
        amount=345.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0034"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0034", candidate_count=4)
