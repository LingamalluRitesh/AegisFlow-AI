"""
Integration Scenario Test Matrix #08
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_08():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0008",
        user_id="usr_scenario_0008",
        source_account_id="acct_src_0008",
        target_account_id="acct_tgt_0008",
        amount=85.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0008"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0008", candidate_count=4)
