"""
Integration Scenario Test Matrix #03
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_03():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0003",
        user_id="usr_scenario_0003",
        source_account_id="acct_src_0003",
        target_account_id="acct_tgt_0003",
        amount=35.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0003"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0003", candidate_count=4)
