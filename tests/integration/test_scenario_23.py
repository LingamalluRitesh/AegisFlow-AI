"""
Integration Scenario Test Matrix #23
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_23():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0023",
        user_id="usr_scenario_0023",
        source_account_id="acct_src_0023",
        target_account_id="acct_tgt_0023",
        amount=235.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0023"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0023", candidate_count=4)
