"""
Integration Scenario Test Matrix #14
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_14():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0014",
        user_id="usr_scenario_0014",
        source_account_id="acct_src_0014",
        target_account_id="acct_tgt_0014",
        amount=145.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0014"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0014", candidate_count=4)
