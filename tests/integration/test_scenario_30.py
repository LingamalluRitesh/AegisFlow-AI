"""
Integration Scenario Test Matrix #30
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_30():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0030",
        user_id="usr_scenario_0030",
        source_account_id="acct_src_0030",
        target_account_id="acct_tgt_0030",
        amount=305.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0030"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0030", candidate_count=4)
