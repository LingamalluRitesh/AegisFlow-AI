"""
Integration Scenario Test Matrix #32
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_32():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0032",
        user_id="usr_scenario_0032",
        source_account_id="acct_src_0032",
        target_account_id="acct_tgt_0032",
        amount=325.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0032"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0032", candidate_count=4)
