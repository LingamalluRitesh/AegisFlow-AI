"""
Integration Scenario Test Matrix #22
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_22():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0022",
        user_id="usr_scenario_0022",
        source_account_id="acct_src_0022",
        target_account_id="acct_tgt_0022",
        amount=225.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0022"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0022", candidate_count=4)
