"""
Integration Scenario Test Matrix #41
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_41():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0041",
        user_id="usr_scenario_0041",
        source_account_id="acct_src_0041",
        target_account_id="acct_tgt_0041",
        amount=415.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0041"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0041", candidate_count=4)
