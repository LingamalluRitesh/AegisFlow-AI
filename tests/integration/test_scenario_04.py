"""
Integration Scenario Test Matrix #04
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_04():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0004",
        user_id="usr_scenario_0004",
        source_account_id="acct_src_0004",
        target_account_id="acct_tgt_0004",
        amount=45.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0004"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0004", candidate_count=4)
