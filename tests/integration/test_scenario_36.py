"""
Integration Scenario Test Matrix #36
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_36():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0036",
        user_id="usr_scenario_0036",
        source_account_id="acct_src_0036",
        target_account_id="acct_tgt_0036",
        amount=365.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0036"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0036", candidate_count=4)
