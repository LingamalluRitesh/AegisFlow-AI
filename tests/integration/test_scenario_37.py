"""
Integration Scenario Test Matrix #37
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_37():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0037",
        user_id="usr_scenario_0037",
        source_account_id="acct_src_0037",
        target_account_id="acct_tgt_0037",
        amount=375.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0037"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0037", candidate_count=4)
