"""
Integration Scenario Test Matrix #26
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_26():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0026",
        user_id="usr_scenario_0026",
        source_account_id="acct_src_0026",
        target_account_id="acct_tgt_0026",
        amount=265.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0026"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0026", candidate_count=4)
