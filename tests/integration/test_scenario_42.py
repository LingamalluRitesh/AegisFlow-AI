"""
Integration Scenario Test Matrix #42
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_42():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0042",
        user_id="usr_scenario_0042",
        source_account_id="acct_src_0042",
        target_account_id="acct_tgt_0042",
        amount=425.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0042"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0042", candidate_count=4)
