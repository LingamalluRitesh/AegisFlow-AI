"""
Integration Scenario Test Matrix #10
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_10():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0010",
        user_id="usr_scenario_0010",
        source_account_id="acct_src_0010",
        target_account_id="acct_tgt_0010",
        amount=105.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0010"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0010", candidate_count=4)
