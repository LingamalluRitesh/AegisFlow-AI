"""
Integration Scenario Test Matrix #45
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_45():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0045",
        user_id="usr_scenario_0045",
        source_account_id="acct_src_0045",
        target_account_id="acct_tgt_0045",
        amount=455.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0045"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0045", candidate_count=4)
