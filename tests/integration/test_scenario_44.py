"""
Integration Scenario Test Matrix #44
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_44():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0044",
        user_id="usr_scenario_0044",
        source_account_id="acct_src_0044",
        target_account_id="acct_tgt_0044",
        amount=445.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0044"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0044", candidate_count=4)
