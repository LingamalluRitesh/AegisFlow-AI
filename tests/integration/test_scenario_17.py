"""
Integration Scenario Test Matrix #17
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_17():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0017",
        user_id="usr_scenario_0017",
        source_account_id="acct_src_0017",
        target_account_id="acct_tgt_0017",
        amount=175.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0017"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0017", candidate_count=4)
