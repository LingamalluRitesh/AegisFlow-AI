"""
Integration Scenario Test Matrix #40
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_40():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0040",
        user_id="usr_scenario_0040",
        source_account_id="acct_src_0040",
        target_account_id="acct_tgt_0040",
        amount=405.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0040"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0040", candidate_count=4)
