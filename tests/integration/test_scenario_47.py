"""
Integration Scenario Test Matrix #47
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_47():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0047",
        user_id="usr_scenario_0047",
        source_account_id="acct_src_0047",
        target_account_id="acct_tgt_0047",
        amount=475.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0047"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0047", candidate_count=4)
