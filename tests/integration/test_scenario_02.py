"""
Integration Scenario Test Matrix #02
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_02():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0002",
        user_id="usr_scenario_0002",
        source_account_id="acct_src_0002",
        target_account_id="acct_tgt_0002",
        amount=25.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0002"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0002", candidate_count=4)
