"""
Integration Scenario Test Matrix #12
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_12():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0012",
        user_id="usr_scenario_0012",
        source_account_id="acct_src_0012",
        target_account_id="acct_tgt_0012",
        amount=125.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0012"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0012", candidate_count=4)
