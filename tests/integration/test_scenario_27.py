"""
Integration Scenario Test Matrix #27
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_27():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0027",
        user_id="usr_scenario_0027",
        source_account_id="acct_src_0027",
        target_account_id="acct_tgt_0027",
        amount=275.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0027"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0027", candidate_count=4)
