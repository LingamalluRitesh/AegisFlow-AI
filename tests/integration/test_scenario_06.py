"""
Integration Scenario Test Matrix #06
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_06():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0006",
        user_id="usr_scenario_0006",
        source_account_id="acct_src_0006",
        target_account_id="acct_tgt_0006",
        amount=65.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0006"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0006", candidate_count=4)
