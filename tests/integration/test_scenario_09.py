"""
Integration Scenario Test Matrix #09
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_09():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0009",
        user_id="usr_scenario_0009",
        source_account_id="acct_src_0009",
        target_account_id="acct_tgt_0009",
        amount=95.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0009"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0009", candidate_count=4)
