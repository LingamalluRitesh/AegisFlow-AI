"""
Integration Scenario Test Matrix #01
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_01():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0001",
        user_id="usr_scenario_0001",
        source_account_id="acct_src_0001",
        target_account_id="acct_tgt_0001",
        amount=15.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0001"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0001", candidate_count=4)
