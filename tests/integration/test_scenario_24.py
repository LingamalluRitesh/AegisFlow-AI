"""
Integration Scenario Test Matrix #24
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_24():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0024",
        user_id="usr_scenario_0024",
        source_account_id="acct_src_0024",
        target_account_id="acct_tgt_0024",
        amount=245.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0024"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0024", candidate_count=4)
