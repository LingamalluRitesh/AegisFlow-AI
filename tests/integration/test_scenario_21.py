"""
Integration Scenario Test Matrix #21
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_21():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0021",
        user_id="usr_scenario_0021",
        source_account_id="acct_src_0021",
        target_account_id="acct_tgt_0021",
        amount=215.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0021"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0021", candidate_count=4)
