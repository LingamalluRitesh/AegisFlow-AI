"""
Integration Scenario Test Matrix #28
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_28():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0028",
        user_id="usr_scenario_0028",
        source_account_id="acct_src_0028",
        target_account_id="acct_tgt_0028",
        amount=285.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0028"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0028", candidate_count=4)
