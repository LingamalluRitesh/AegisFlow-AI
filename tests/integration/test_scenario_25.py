"""
Integration Scenario Test Matrix #25
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_25():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0025",
        user_id="usr_scenario_0025",
        source_account_id="acct_src_0025",
        target_account_id="acct_tgt_0025",
        amount=255.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0025"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0025", candidate_count=4)
