"""
Integration Scenario Test Matrix #15
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_15():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0015",
        user_id="usr_scenario_0015",
        source_account_id="acct_src_0015",
        target_account_id="acct_tgt_0015",
        amount=155.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0015"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0015", candidate_count=4)
