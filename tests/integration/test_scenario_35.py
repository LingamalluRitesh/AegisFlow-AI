"""
Integration Scenario Test Matrix #35
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_35():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0035",
        user_id="usr_scenario_0035",
        source_account_id="acct_src_0035",
        target_account_id="acct_tgt_0035",
        amount=355.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0035"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0035", candidate_count=4)
