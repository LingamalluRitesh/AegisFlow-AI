"""
Integration Scenario Test Matrix #46
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_46():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0046",
        user_id="usr_scenario_0046",
        source_account_id="acct_src_0046",
        target_account_id="acct_tgt_0046",
        amount=465.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0046"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0046", candidate_count=4)
