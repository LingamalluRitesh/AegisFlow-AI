"""
Integration Scenario Test Matrix #43
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_43():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0043",
        user_id="usr_scenario_0043",
        source_account_id="acct_src_0043",
        target_account_id="acct_tgt_0043",
        amount=435.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0043"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0043", candidate_count=4)
