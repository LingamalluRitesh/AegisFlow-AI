"""
Integration Scenario Test Matrix #11
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_11():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0011",
        user_id="usr_scenario_0011",
        source_account_id="acct_src_0011",
        target_account_id="acct_tgt_0011",
        amount=115.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0011"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0011", candidate_count=4)
