"""
Integration Scenario Test Matrix #29
...
@pytest.mark.asyncio
async def test_end_to_end_scenario_29():
    tx = TransactionEvent(
        transaction_id="tx_scenario_0029",
        user_id="usr_scenario_0029",
        source_account_id="acct_src_0029",
        target_account_id="acct_tgt_0029",
        amount=295.0,
        currency="USD",
        channel="mobile_app",
    )
    ...
    assert decision.transaction_id == "tx_scenario_0029"
    assert decision.evaluation_latency_ms < 1000.0

    from backend.core.types import RecommendationRequest
    rec_req = RecommendationRequest(user_id="usr_scenario_0029", candidate_count=4)
